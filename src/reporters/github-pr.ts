...
export async function postPrReview(
  ...
  return {
    ...
    criticalOnDiff,
    severityCountsInDiff: severityBreakdown(inDiff),
...
export function evaluatePrGates(
  prResult: PrPostResult,
  totalCostUSD: number,
  gates: SecureReviewConfig['gates'],
): PrGateDecision {
  const reasons: string[] = [];

  if (gates.block_on_new_critical && prResult.severityCountsTouched.CRITICAL > 0) {
    reasons.push(
      `${prResult.severityCountsTouched.CRITICAL} CRITICAL finding(s) on changed files`,
    );
  }
  if (gates.block_on_new_high && prResult.severityCountsTouched.HIGH > 0) {
    reasons.push(`${prResult.severityCountsTouched.HIGH} HIGH finding(s) on changed files`);
  }
  if (gates.max_cost_usd > 0 && totalCostUSD > gates.max_cost_usd) {
    reasons.push(`cost cap hit: $${totalCostUSD.toFixed(2)} > $${gates.max_cost_usd.toFixed(2)}`);
  }
...
- **npm-audit**: ${output.sast.npmAudit.ran ? output.sast.npmAudit.count : 'skipped'}`;
  ...
  return `${reviewStatusLine(output)}

## 🔒 Secure Review — multi-model security audit
...
function reviewStatusLine(output: ReviewModeOutput): string {
